    ...
    async def call(self, context, **kwargs) -> ToolExecResult:
        ...
        try:
            if self.name == "txcm_status":
                return await self.plugin.tool_status()
            if self.name == "txcm_list_tools":
                return await self.plugin.tool_list_tools(**kwargs)
            if self.name == "txcm_get_tool_schema":
                return await self.plugin.tool_get_tool_schema(**kwargs)
            if self.name == "txcm_list_guilds":
                return await self.plugin.tool_list_guilds()
            if self.name == "txcm_guild_channels":
                return await self.plugin.tool_guild_channels(**kwargs)
            if self.name == "txcm_search_feeds":
                return await self.plugin.tool_search_feeds(**kwargs)
            ...
            if self.name == "txcm_get_cli_mapping":
                return await self.plugin.tool_get_cli_mapping(**kwargs)
            if self.name == "txcm_call_cli_command":
                return await self.plugin.tool_call_cli_command(**kwargs)
            if self.name == "txcm_endpoint_guide":
                return await self.plugin.tool_endpoint_guide(**kwargs)
            return json.dumps(
                {
                    "ok": False,
